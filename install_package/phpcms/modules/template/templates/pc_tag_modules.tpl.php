...
<!--
	$(function(){
		...
		$("#return").formValidator({onshow:"<?php echo L("please_enter_the_data_returned_value_by_default")?>£ºdata¡£",onfocus:"<?php echo L("please_enter_the_data_returned_value_by_default")?>£ºdata¡£",empty:true});
...
<table width="100%"  class="table_form">
	  <tr>
    <th width="80"><?php echo L("module")?>£º</th>
    <td class="y-bg"><?php echo $op?></td>
  </tr>
    <tr>
    <th width="80"><?php echo L('operation')?>£º</th>
  ...
  <?php if(isset($v['ajax']['name'])  && !empty($v['ajax']['name'])) {?>
  	  <tr>
  	  	<th width="80"><?php echo $v['ajax']['name']?>£º<?php if(isset($_GET[$v['ajax']['id']]) && !empty($_GET[$v['ajax']['id']])) echo '<script type="text/javascript">$.get(\'?m=template&c=file&a=public_ajax_get\', { html: \''.$_GET[$k].'\', id:\''.$v['ajax']['id'].'\', value:\''.$_GET[$v['ajax']['id']].'\', action: \''.$v['ajax']['action'].'\', op: \''.$op.'\', style: \'default\'}, function(data) {$(\'#'.$k.'_td\').html(data)});</script>'?></th>
...
<fieldset>
	<legend><?php echo L('vlan')?></legend>
		<table width="100%"  class="table_form">
	  <tr>
    <th width="80"><?php echo L("public_allowpageing")?>£º</th>
