...
<fieldset>
  ...
  <?php if ($module):?>
	...
	<?php if(isset($v['ajax']['name'])  && !empty($v['ajax']['name'])) {?>
  	  <tr>
  	  	<th width="80"><?php echo $v['ajax']['name']?>£º<?php if(isset($_GET[$v['ajax']['id']]) && !empty($_GET[$v['ajax']['id']])) echo '<script type="text/javascript">$.get(\'?m=template&c=file&a=public_ajax_get\', { html: \''.$_GET[$k].'\', id:\''.$v['ajax']['id'].'\', value:\''.$_GET[$v['ajax']['id']].'\', action: \''.$v['ajax']['action'].'\', op: \''.$module.'\', style: \'default\'}, function(data) {$(\'#'.$k.'_td\').html(data)});</script>'?></th>
  ...
  <tr>
		<th valign="top"><?php echo L('over_dbsource')?>£º</th>
  ...
  <?php else :?>
  <tr>
		<th valign="top"><?php echo L('block').L('name')?>£º</th>
...
<fieldset>
	...
	<tr>
		<th width="80"><?php echo L('ispage')?>£º</th>
